...
const AppRoutes = () => {
  ...
  useEffect(() => {
    const summedEntries = list.reduce((acc, item) => acc + item.entrada, 0);
    setTotalEntries(summedEntries);

    const summedOutflows = list.reduce((acc, item) => acc + item.saida, 0);
    setTotalOutflows(summedOutflows);

    const summedTotal = list.reduce((acc, item) => acc + item.total, 0);
  ...
  return (
    <Router>
      <Navbar />
      <Routes>
        <Route path="/" element={<Home totalEntries={totalEntries} totalOutflows={totalOutflows} total={total} />} />
        <Route path="/transactions" element={<Transactions list={list} />} />
      </Routes>
      <FooterDev />
    </Router>
  );
